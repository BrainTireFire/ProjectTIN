...
export default function ReviewDetails() {
  ...
  return (
    <div>
      <Header as="h2">
        Reviews of the {reviewsDetailsOfAlcoholQuery.data?.name}
      </Header>
          ...
          <Card key={review.review._id} className="review-card">
            <Card.Content>
              <Card.Header>
                {review.review?.user ? (
                  <p>
                    {t("name")}: {review.review?.user?.name}
                  </p>
                ) : (
                  <p>{t("deleteUser")}</p>
                )}
            ...
            {currentUser.data &&
            currentUser.data?._id === review.review?.user?._id ? (
              <Button
                onClick={() => deleteReview(review.review._id)}
                color="red"
              >
                {t("delete")}
              </Button>
